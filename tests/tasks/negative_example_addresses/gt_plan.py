...
def main() -> None:
    ...
    df = pd.read_csv("tests/tasks/datalake/adventure_works/Address_SEP_table_45.csv")[['City', 'PostalCode']].copy()
    for _col in df.columns:
        if is_string_dtype(df[_col]):
            df[_col] = df[_col].astype(str).str.lower()
    df2 = pd.read_csv("tests/tasks/datalake/adventure_works/Address_SEP_table_45.csv")[['City', 'StateProvinceID']].copy()
    ...
    k = 50
    plan = Plan()
    plan.add("query_exclusive", Seekers.MC(df2[['City', 'StateProvinceID']], k*4))
    plan.add("query_inclusive", Seekers.MC(df[['City', 'PostalCode']], k*2))
