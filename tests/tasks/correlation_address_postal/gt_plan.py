...
def main() -> None:
    ...
    df = pd.read_csv("tests/tasks/datalake/adventure_works/Address_SEP_table_45.csv")[['City']].copy()
    for _col in df.columns:
        if is_string_dtype(df[_col]):
            df[_col] = df[_col].astype(str).str.lower()
    df2 = pd.read_csv("tests/tasks/datalake/adventure_works/Address_SEP_table_45.csv")[['StateProvinceID']].copy()
